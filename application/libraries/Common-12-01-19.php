...
class Common {
	...
	public function get_join_all_record_search($tables,$joins,$rows="*",$groupBy="",$order_by=array(),$wh = array(),$keywords='',$field_array=array(),$params = array())
	{
		$this->CI->db->select($rows);
		$this->CI->db->from($tables[0]);
		foreach ($joins as $key => $value) {
	...
	public function get_all_join_record($tables,$joins,$rows="*",$groupBy="",$order_by=array(),$wh = array(),$keywords='',$field_array=array(),$params = array())
	{
		$this->CI->db->select($rows);

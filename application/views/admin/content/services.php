...
<section class="content-header">
   ...
   if(!empty($cms)) {
   ?>
   <h1><i class="fa fa-plus"></i>&nbsp;Edit Services</h1>
   <?php
   }
   else {
   ?>
   <h1><i class="fa fa-plus"></i>&nbsp;Add Services</h1>
   <?php
   }
   ?>
</section>
...
<section class="content">
   ...
   <div class="box box-info">
      ...
      if(!empty($cms)) {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_services/'.$cms['id'] ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      else {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_services' ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      ?>

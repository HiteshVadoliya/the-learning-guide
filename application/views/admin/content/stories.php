...
<section class="content-header">
   ...
   if(!empty($cms)) {
   ?>
   <h1><i class="fa fa-plus"></i>&nbsp;Edit Stories</h1>
   <?php
   }
   else {
   ?>
   <h1><i class="fa fa-plus"></i>&nbsp;Add Stories</h1>
   <?php
   }
   ?>
</section>
...
<section class="content">
   ...
   <div class="box box-info">
      ...
      if(!empty($cms)) {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_stories/'.$cms['id'] ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      else {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_stories' ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      ?>

...
<section class="content-header">
   ...
   if(!empty($cms)) {
   ?>
   <h1><i class="fa fa-plus"></i>&nbsp;Edit Team</h1>
   <?php
   }
   else {
   ?>
...
<section class="content">
   ...
   <div class="box box-info">
      ...
      if(!empty($cms)) {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_team/'.$cms['id'] ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      else {
      ?>
      <form role="form" action="<?= ADMIN_LINK.'Content/create_update_team' ?>" method="post" id="bulletinForm" enctype="multipart/form-data">
      <?php
      }
      ?>
